...
fun showToast(text: String?) {
    showToast(context = DormitoryApplication.CONTEXT, text = text)
}

fun showToast(context: Context = DormitoryApplication.CONTEXT, text: String?) {
 ...
 */
fun showToast(context: Context = DormitoryApplication.CONTEXT, resId: Int) {
    if (Thread.currentThread() === Looper.getMainLooper().thread) {
 ...
 * @param text String，显示的内容
 */
fun showLongToast(context: Context? = DormitoryApplication.CONTEXT, text: String?) {
 ...
 * @param resId int，显示内容的字符串索引
 */
fun showLongToast(context: Context? = DormitoryApplication.CONTEXT, resId: Int) {
...
private fun showToast(context: Context? = DormitoryApplication.CONTEXT, text: String?, duration: Int) {
    if (TextUtils.isEmpty(text)) return
    cancelToast()
    if (mToast == null) {
...
fun showToast(context: Context? = DormitoryApplication.CONTEXT, res: Int, duration: Int) {
    cancelToast()
    if (mToast == null) {
